...
int main(void)
{
  ...
  rewind(reads);
  char hold3; // utility variable
  
  for (int i = 0; i < nRead; i++)
  {
    // these are where each should start to print into the 
    // here we could fortify another file that keeps the prefixes, just an idea
    // fseek(reads, lengths[i] - min, SEEK_CUR);
    // instead of seeking curr try to get the start
    for (int k = 0; k < lengths[i] - min; k++)
    {
      fscanf(reads, "%c", &hold3);
      fprintf(linkPre, "%c", hold3);
    }
    fprintf(linkPre, "&\n");
